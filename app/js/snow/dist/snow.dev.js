...
var Snow = function Snow(options) {
  ...
  document.getElementById(options.id).style.zIndex = 100;
  document.getElementById(options.id).style.pointerEvents = "none"; //create canvas

  this.canvas = document.createElement("canvas"); //add random number to change canvas id

  this.canvas.width = window.innerWidth;
  this.canvas.height = window.innerHeight;
  document.getElementById(options.id).appendChild(this.canvas); //get theme
  ...
  this.snowfall = function () {
    ...
    requestAnimationFrame(function () {
      return _this.snowfall();
    });

    if (this.go) {
      //clear canvas
      var context = this.canvas.getContext('2d');
      context.clearRect(0, 0, this.canvas.width, this.canvas.height); //update snowflakes
...
var Snowflake = function Snowflake(canvas, theme, min, max) {
  //snowflake elements
  this.radius = random(min, max);
  this.x = random(0, canvas.width);
  this.y = random(-20, -800);
  this.Vy = random(1, 2);
  console.log(this.canvas); //set default

  this.color = "#FFF"; //apply theme
